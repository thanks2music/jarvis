...
- **`/doctor` で skills と context ファイルを right-size する**（**v2.1.206 以降**）。チェックイン済み CLAUDE.md に対して「**コードから導出できる内容（ディレクトリ構成・依存一覧・アーキテクチャ概要）を削り、落とし穴・理由・ツール既定と異なる規約を残す**」trim を提案する。これは公式が明示的に推奨する運用手段である。
