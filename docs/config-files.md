...
> **実務インパクト**: Max プラン + v2.1.233 以降であれば**バージョン要件を満たしており、対話セッションは既に auto mode が既定**である。一方、**`claude -p` を使うバッチ実行は除外条件 3 に該当し、従来どおり Manual 既定のまま**である点に注意する（`-p` で auto を使いたい場合は `--permission-mode auto` を明示する）。`~/.claude/settings.json` で `defaultMode` を明示している場合はその設定が優先されるため、意図した挙動を固定したいなら明示設定を入れておくのが確実である。
>
...
> | **`CLAUDE_CODE_ENABLE_TODO_TOOLS`** | 新世代モデルで既定除外された task-tracking ツール群を**入れる**（`=1`、起動前に export）。**現役の変数である**。別軸の **`CLAUDE_CODE_ENABLE_TASKS`**（ツールがあるセッション内で Task 4 種と旧 `TodoWrite` のどちらを使うかを選ぶ。既定 Task 4 種、`=0` で `TodoWrite`）と共存する。詳細は [sub-agents.md](sub-agents.md) の「Task / Todo ツールのモデル別提供状況」を参照 | v2.1.233 |
